...
addbtn= Button(leftframe, text= "+ Add", command= browsesong)
...
lengthlabel = Label(topframe, text='Total Length :- 00:00')  # giving parameters for label and label is a widget
lengthlabel.pack(pady=5)  # we cannot add widgets to window without packing them

currenttimelabel = Label(topframe, text='Current time : --:--',relief=GROOVE)  # giving parameters for label and label is a widget
...
def setvol (val):  # func for volume control and val argument is used by python to send the value from the command function
    volume = int(val) / 100  # to make it in a value between 0 to 1
...
playbtn = Button(middleframe, image=playphoto, command=playmusic)  # to make the play image work as button
playbtn.grid(row=0, column=1, padx=10)  # again packing it to display the widget

stopphoto = PhotoImage(file='stopp.png')  # to add img in this function
stopbtn = Button(middleframe, image=stopphoto, command=stopmusic)  # to make the stop button work as a button
stopbtn.grid(row=0, column=2, padx=10)  # packing widget so it comes below the last widget

pausephoto = PhotoImage(file='pause.png')  # to add img in this function
pausebtn = Button(middleframe, image=pausephoto, command=pausemusic)  # to make the pause button work as a button
pausebtn.grid(row=0, column=3, padx=10)  # packing widget so it comes below the last widget
...
rewindbtn = Button(bottomframe, image=rewindphoto, command=rewindmusic)  # to make the pause button work as a button
rewindbtn.grid(row=0, column=0)  # packing widget so it comes below the last widget
mutephoto= PhotoImage(file='mute.png')
volphoto= PhotoImage(file='volume.png')
volbtn= Button(bottomframe,image=volphoto,command= mutemusic)
volbtn.grid(row=0,column=1,padx=10)


scale = Scale(bottomframe, from_=0, to=100, orient=HORIZONTAL, command=setvol)  # neeche vala same
